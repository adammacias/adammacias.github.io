...
module.exports = function(grunt) {
    ...
    var jmConfig = {
        ...
        uglify: {
            bootstrap: {
                files: {
                    '<%= dirs.js %>/vendor/bootstrap.min.js': [
                      '<%= dirs.js %>/bootstrap/util.js',
            ...
            dist: {
                files: {
                    '<%= dirs.js %>/main.min.js': [
                        //'<%= dirs.plugins %>/header-intro-effect/header-intro-effect.min.js',
                        '<%= dirs.plugins %>/nicescroll/jquery.nicescroll.min.js',

                        '<%= dirs.js %>/vendor/tether.min.js',
                        '<%= dirs.js %>/vendor/bootstrap.min.js',
                        '<%= dirs.js %>/vendor/jquery.gifplayer.min.js',

                        '<%= dirs.plugins %>/lightgallery/js/lightgallery.min.js',
                        '<%= dirs.plugins %>/lightgallery/js/lg-thumbnail.min.js',
                        '<%= dirs.plugins %>/lightgallery/js/lg-fullscreen.min.js',

                        '<%= dirs.js %>/vendor/jquery.timeago.js',
                        '<%= dirs.js %>/vendor/jquery.timeago.pt-br.js',
                        '<%= dirs.js %>/vendor/jquery.fitvids.js',
            ...
            home: {
                files: {
                    '<%= dirs.plugins %>/ambb/ambb.min.js': [
                        '<%= dirs.plugins %>/ambb/dat.gui.min.js',
                        '<%= dirs.plugins %>/ambb/ambb.js'
                    ]
                }
            },
            plugin_header_intro_effect: {
                files: {
                    '<%= dirs.plugins %>/header-intro-effect/header-intro-effect.min.js': [
                        '<%= dirs.plugins %>/header-intro-effect/header-intro-effect.js'
                    ]
                }
            }
        },
